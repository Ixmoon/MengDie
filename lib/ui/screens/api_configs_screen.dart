...
class ApiConfigsScreen extends ConsumerWidget {
  ...
  void _showApiConfigDialog(
    ...
    final modelController = TextEditingController(
      text: existingConfig?.model ?? '',
    );
    final apiKeyController = TextEditingController(
      text: existingConfig?.apiKey ?? '',
    );
    final baseUrlController = TextEditingController(
      text: existingConfig?.baseUrl ?? '',
    );
    final maxTokensController = TextEditingController(
      text: existingConfig?.maxOutputTokens?.toString() ?? '',
    );
    final stopSequencesController = TextEditingController(
      text: existingConfig?.stopSequences?.join(', ') ?? '',
    );
    final thinkingBudgetController = TextEditingController(
      text: existingConfig?.thinkingBudget?.toString() ?? '',
    );
    final toolConfigController = TextEditingController(
      text: existingConfig?.toolConfig ?? '',
    );
    final toolChoiceController = TextEditingController(
      text: existingConfig?.toolChoice ?? '',
    );
    ...
    var reasoningEffort =
        existingConfig?.reasoningEffort ?? OpenAIReasoningEffort.auto;
    var useDefaultSafetySettings =
            ...
            return AlertDialog(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: selectedApiType == LlmType.openai
                              ? 'API Key (必填)'
                              : '专用 API Key (可选)',
                          hintText: selectedApiType == LlmType.gemini
                              ? '留空则使用全局密钥池'
                              : '',
                        ),
                        obscureText: true,
                      ...
                      if (selectedApiType == LlmType.openai) ...[
                        SwitchListTile(
                          ...
                          onChanged: (val) =>
                              setDialogState(() => enableReasoningEffort = val),
                        ),
                        if (enableReasoningEffort)
                          DropdownButtonFormField<OpenAIReasoningEffort>(
                            ...
                            decoration: const InputDecoration(
                              labelText: '推理强度 (Reasoning Effort)',
                            ),
                          ),
                        const SizedBox(height: 16),
                      ],
                      ...
                      if (selectedApiType == LlmType.openai) ...[
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(
                            labelText: 'Tool Choice (可选)',
                            hintText: 'auto, none, or JSON',
                          ),
                        ),
                      ],
                      if (selectedApiType == LlmType.gemini) ...[
                      ...
                      SwitchListTile(
                        ...
                        onChanged: (val) =>
                            setDialogState(() => useCustomTemperature = val),
                      ),
                      Slider(
                        value: temperature,
                        min: 0.0,
                        ...
                        onChanged: useCustomTemperature
                            ? (val) => setDialogState(() => temperature = val)
                            : null,
                      ),
                      SwitchListTile(
                        title: const Text('自定义 Top P'),
                        subtitle: Text(
                          useCustomTopP ? topP.toStringAsFixed(2) : 'API 默认',
                        ),
                        value: useCustomTopP,
                        onChanged: (val) =>
                            setDialogState(() => useCustomTopP = val),
                      ),
                      Slider(
                        value: topP,
                        min: 0.0,
                        ...
                        onChanged: useCustomTopP
                            ? (val) => setDialogState(() => topP = val)
                            : null,
                      ),
                      SwitchListTile(
                        title: const Text('自定义 Top K'),
                        subtitle: Text(
                          useCustomTopK ? topK.round().toString() : 'API 默认',
                        ),
                        value: useCustomTopK,
                        onChanged: (val) =>
                            setDialogState(() => useCustomTopK = val),
                      ),
                      Slider(
                        value: topK,
                        min: 1.0,
                        ...
                        onChanged: useCustomTopK
                            ? (val) => setDialogState(() => topK = val)
                            : null,
                      ),
                    ],
                  ),
              ...
              actions: [
                ...
                if (!isNew)
                  TextButton(
                    onPressed: () {
                      final allConfigs = ref
                          .read(apiKeyNotifierProvider)
                          .apiConfigs;
                      final currentName = nameController.text.isNotEmpty
                          ? nameController.text
                      ...
                      if (selectedApiType == LlmType.openai &&
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('OpenAI 配置需要填写 API Key'),
                          ),
                        );
                        return;
                      }
                      ...
                      ref
                          ...
                          .saveConfig(
                            ...
                            apiKey: apiKeyController.text.isNotEmpty
                                ...
                                : null,
                            baseUrl:
                                (selectedApiType == LlmType.openai ||
                                        selectedApiType == LlmType.gemini) &&
                                    baseUrlController.text.isNotEmpty
                                ? baseUrlController.text
                                : null,
                            useCustomTemperature: useCustomTemperature,
                            temperature: useCustomTemperature
                                ? temperature
                                : null,
                            ...
                            stopSequences: stopSequencesController.text
                                .split(',')
                                .map((s) => s.trim())
                            ...
                            thinkingBudget: int.tryParse(
                              thinkingBudgetController.text,
                            ),
                            toolConfig: toolConfigController.text.isNotEmpty
                                ? toolConfigController.text
                                : null,
                            toolChoice: toolChoiceController.text.isNotEmpty
                                ? toolChoiceController.text
                                : null,
                ...
                TextButton(
                  onPressed: () {
                    final allConfigs = ref
                        .read(apiKeyNotifierProvider)
                        .apiConfigs;
                    final configName = nameController.text.isNotEmpty
                        ? nameController.text
                        : modelController.text;
                    ...
                    if (allConfigs.any(
                      (c) => c.name == configName && c.id != existingConfig?.id,
                    )) {
                      ScaffoldMessenger.of(context).showSnackBar(
