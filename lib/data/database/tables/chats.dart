...
class Chats extends Table {
  ...
  IntColumn get updatedAt => integer()
      .map(const MicrosecondDateTimeConverter())
      .clientDefault(() => DateTime.now().toUtc().microsecondsSinceEpoch)();
  TextColumn get coverImageBase64 =>
      text().nullable()(); // 新增：用于存储封面图片的 Base64 字符串
  ...
  TextColumn get contextSummary =>
      text().nullable()(); // Stores the last summary from pre-processing
  IntColumn get lastSummarizedMessageId =>
      integer().nullable()(); // ID of the last message included in the summary
  ...
  TextColumn get helpMeReplyTriggerMode =>
      text().map(const HelpMeReplyTriggerModeConverter()).nullable()();
  // --- End ---
