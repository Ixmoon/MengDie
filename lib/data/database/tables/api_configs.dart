...
class ApiConfigs extends Table {
  ...
  TextColumn get apiType =>
      text().map(const LlmTypeConverter())(); // Gemini or OpenAI
  TextColumn get model => text()(); // 模型名称, e.g., 'gemini-1.5-pro', 'gpt-4'
  TextColumn get apiKey =>
      text().nullable()(); // API Key (对于 Gemini, 这是全局的, 此处可为空)
  TextColumn get baseUrl =>
      text().nullable()(); // Base URL (主要用于 OpenAI 兼容 API)
  ...
  BoolColumn get useDefaultSafetySettings =>
