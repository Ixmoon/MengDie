...
void main() {
  group('BackgroundTasks Summarization Logic', () {
    ...
    setUp(() {
      ...
      when(
        mockRef.read(contextXmlServiceProvider),
      ).thenReturn(mockContextXmlService);
      when(
        mockRef.read(currentChatProvider(1)),
      ).thenReturn(AsyncValue.data(mockChat));
      when(
        mockRef.read(chatStateNotifierProvider(1).notifier),
      ).thenReturn(mockChatStateNotifier);
      when(
        mockChatStateNotifier.getEffectiveApiConfig(
          specificConfigId: anyNamed('specificConfigId'),
        ),
    ...
    test(
      'triggers summarization with correct chunk content and order',
      () async {
        ...
        when(
          mockContextXmlService.predictContextUsage(chatId: anyNamed('chatId')),
        ).thenAnswer(
          (_) async => ContextPredictionResult(
            willExceed: true,
        ...
        when(
          mockContextXmlService.buildApiRequestContext(
          ...
          final List<Message> history =
              invocation.namedArguments[#historyOverride] as List<Message>;
          final chunkSize = 20;
          if (history.isEmpty) {
        ...
        when(
          mockLlmService.sendMessageOnce(
            llmContext: anyNamed('llmContext'),
            apiConfig: anyNamed('apiConfig'),
          ),
        ).thenAnswer((invocation) async {
          final List<LlmContent> llmContext =
              invocation.namedArguments[#llmContext] as List<LlmContent>;
        ...
        final captured = verify(
          mockChatRepository.saveChat(captureAny),
        ).captured;
...
class TestBackgroundTasksMixin extends StateNotifier<ChatScreenState>
    with UiStateManager, BackgroundTasks {
  @override
  final Ref ref;
  @override
  final int chatId;

  TestBackgroundTasksMixin(this.ref, this.chatId)
  ...
  Future<String> callSummarizeMessages(
    List<Message> messages,
    String? existingSummary, {
    List<Message>? followingMessages,
  }) {
    return super.summarizeMessages(
      messages,
      existingSummary,
