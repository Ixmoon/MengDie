...
import '../../../../ui/screens/chat_settings_screen.dart'
    show defaultHelpMeReplyPrompt;
...
mixin SpecialActions on StateNotifier<ChatScreenState> {
  ...
  Future<void> generateHelpMeReply({
    ...
    try {
      final bool hasExistingSuggestions =
          state.helpMeReplySuggestions != null &&
          state.helpMeReplySuggestions!.isNotEmpty;

      if (!forceRefresh && hasExistingSuggestions) {
        if (onSuggestionsReady != null) {
          final currentPage =
              state.helpMeReplySuggestions![state.helpMeReplyPageIndex];
      ...
      try {
        final apiConfig = getEffectiveApiConfig(
          specificConfigId: chat.helpMeReplyApiConfigId,
        );
        ...
        final newSuggestions = RegExp(
          r'^\s*\d+\.\s*(.*)',
          multiLine: true,
        ).allMatches(generatedText).map((m) => m.group(1)!.trim()).toList();
        final finalSuggestions = newSuggestions.isNotEmpty
            ? newSuggestions
            : [generatedText.trim()];
        ...
        if (mounted) {
          ...
          for (
            var i = 0;
            i < finalSuggestions.length;
            i += _kSuggestionsPerPage
          ) {
            final end = (i + _kSuggestionsPerPage < finalSuggestions.length)
                ? i + _kSuggestionsPerPage
          ...
          final currentPages = clearPreviousSuggestions
              ? <List<String>>[]
              : (state.helpMeReplySuggestions ?? []);
          final updatedPages = List<List<String>>.from(currentPages)
            ..addAll(newPages);
  ...
  Future<String> executeSpecialAction({
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (response.isSuccess && response.parts.isNotEmpty) {
          final generatedText = response.parts
              .map((p) => p.text ?? "")
              .join("\n");
    ...
    throw Exception(
      "Special action '$actionType' failed after $maxRetries attempts.",
    );
  }
}
